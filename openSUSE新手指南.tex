...
运行\command{zypper update}的时候常常会遇到以下情况：“将不会安装以下 51 个软件包的更新：blahblahblah”，若对此有疑问请参见\href{https://forum.suse.org.cn/viewtopic.php?t=2777&p=21896}{为什么有些包不会被更新}
\section{本地化配置}
...
\section{系统设置}
\subsection{字体设置与管理}
正式文档排版一般都需要宋体、黑体、仿宋、楷体这几种字体，从\href{http://pan.baidu.com/s/1mgiHWmO}{Helloworld.rpm}下载好后点击即可安装。开源的字体如Ubuntu字体(\soft{ubuntu-fonts})和文泉驿微米黑(\soft{wqy-microhei-fonts})等则可以直接在源中找到甚至默认已经安装好了。
...
对于root程序的字体，你需要在Konsole中以root运行\command{systemsettings}再单独设置。在openSUSE中为了能够以root运行相关图形程序，需要使用\command{kdesu}来代替\command{sudo}。
...
添加部件时你既可以双击添加到面板，也可以托拽到桌面去。图\ref{mydesk}就是我的桌面截图。
\begin{figure}[htb]
 \centering
